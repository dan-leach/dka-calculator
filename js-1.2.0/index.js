$(document).ready(function(){ //fires once page loaded
	//alert('Hi - I\'m actually working on this right now - so some of the behaviour might be a bit unpredictable. You might want to check back later.');

	clearForm(); //ensures that the form is blank if navigated to using back key from submit page
	
	add_auditID(); //automatically fills the auditID field

    function clearForm(){ //clears the form
		$('#name').val("");
		$('#dob').val("");
		$('#num').val("");
		$('#protocolStart').val("");
    ...
    $("#testfill").click(function(){//autofills the form with acceptable test patient data for ease during testing (not accessible when Joe Bloggs button hidden in live version)
		...
		$('#centre').val("University Hospitals Bristol");
		checkProgress("centre");
		$('#episode').val("4");
		checkProgress("episode");
	});
    
	$("#btnsubmit").click(function(){ //runs on clicking submit - updates the styling and info messages for all inputs - if correct then submits non-confidential form to server and moves to submit page
	
		//cycles through all the inputs and runs their check functions, and updates their success/error styling. Also counts the number of inputs where the check function returns true
		var correctCount = 0;
		...
		if(correctCount===14){//14 rather than 15 correctCount as region skipped as above
			if (document.getElementById("overrideCheckbox").checked === true){
				bootbox.confirm({
					...
					message: "You should only continue if you are sure " + inputs.weight + "kg is the correct weight and you have considererd using a maximum weight of the 98th centile weight for age as per the care pathway.<br><br>You can proceed with a weight that is outside the expected range, however the calculator has upper limits that cannot be overriden. These are based on a maximum weight of 75kg. Any calculated values that exceed this will be capped as follows:<br><br><ul><li>Daily maintenance volume is capped at 2600mL (Holliday-Segar formula for 75kg)</li><li>Deficit volume is capped at 7500mL for patients with severe DKA (10% dehydration for 75kg)</li><li>Deficit volume is capped at 3750mL for patients with severe DKA (5% dehydration for 75kg)</li><li>Bolus volumes are capped at 750mL (10mL/kg for 75kg)</li><li>Insulin rate is capped at 7.5 Units/hour if insulin rate of 0.1 Units/kg/hour is selected (0.1 Units/kg/hour for 75kg patient)</li><li>Insulin rate is capped at 3.75 Units/hour if insulin rate of 0.05 Units/kg/hour is selected (0.05 Units/kg/hour for 75kg patient)</li></ul><br><strong>Calculations will based on " + inputs.weight + "kg and only capped if they exceed the values above.</strong> Bear in mind that these caps could still allow significantly excessive values especially if your patient is much smaller than 75kg.<br><br>Are you sure you want to proceed?",
					buttons: {
						cancel: {
...
function getInputs(){ //creates an object (inputs) containing the values (or functions returning the values) of all the inputs on the forms ***contains confidential data***
	var inputs = {
		...
		episode: function(){
			var ctrlepisode = document.getElementById("episode");
			var getepisode = ctrlepisode.options[ctrlepisode.selectedIndex].value;
			return getepisode;
		},
		auditID: function(){
...
function checkProgress(triggerctrl){ //runs onBlur of any input on forms (except age) - updates the progress bar, and the styling and info text for the triggering input
	...
	var correctCount = document.getElementsByClassName("has-success").length -1; //-1 to exclude audit ID which should has-success but don't want to include it
		...
		if($.isNumeric(inputs.weight)){
			...
			if (document.getElementById("overrideCheckbox").checked == true){
				...
				if (inputs.weight < limits.lower || inputs.weight > limits.upper)	{
					...
					return "Weight must be within 2 standard deviations of the mean for age (upper limit 75kg) ("+limits.lower+"kg to "+limits.upper+"kg).";
				} else {
					//correct range
...
function checkepisode(inputs){ //the check function for the episode input - returns true if valid, else returns string describing the error to be corrected 
	if(inputs.episode()=== "1" ){ //no selection made
		return "Select an option from the drop-down menu.";
	} else { //selection made
		return true;
	}
...
function checkauditID(inputs){ //the check function for the auditID input - returns true if valid, else returns string describing the error to be corrected 
	if (inputs.auditID() === ""){
		return "Error - audit ID not generated properly.";
	} else {
		return true;
	}
}