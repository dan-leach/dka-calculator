...
const showWorking = {
  click: function () {
    //triggered by user clicking show working
    try {
      var workingDisplay = "";
      for (calc in this.calcs) {
        //builds the HTML components that make up the show working modal content, one calculation at a time
        workingDisplay =
          workingDisplay +
          '<style>.well {margin-bottom: 5px;}.bg-clear {background-color: transparent;}</style><div class="panel panel-info"><div class="panel-heading"><strong>' +
          this.calcs[calc].name +
          '</strong></div><div class="panel-body"><div class="row"><div class="col-md-2"><div class="well well-sm bg-clear"><strong>Formula:</strong></div></div><div class="col-md-10"><div class="well well-sm">' +
          this.calcs[calc].formula() +
          "</div></div>";
        if (!this.calcs[calc].limit().length == 0) {
          //show the limit line only if there is a limit
          workingDisplay =
            workingDisplay +
            '<div class="col-md-2"><div class="well well-sm bg-clear"><strong>Limit*:</strong></div></div><div class="col-md-10"><div class="well well-sm">' +
            this.calcs[calc].limit() +
            "</div></div>";
        }
        workingDisplay =
          workingDisplay +
          '<div class="col-md-2"><div class="well well-sm bg-clear"><strong>Working:</strong></div></div><div class="col-md-10"><div class="well well-sm">' +
          this.calcs[calc].working() +
          '</div></div><div class="col-md-2"><div class="well well-sm bg-clear"><strong>Output:</strong></div></div><div class="col-md-10"><div class="well well-sm">' +
          this.calcs[calc].output() +
          "</div></div></div></div></div>";
      }
      workingDisplay =
        workingDisplay +
        "*limits set based on weight of " +
        settings.caps.weight +
        "kg<br><br>N.B. Insulin should NOT be started immediately. Refer to the BSPED integrated care pathway for how to use these calculated values.";
      bootbox.dialog({
        //show the modal
  ...
  calcs: {
    //the formula, limit (where applicable), working and output for each calculated variable
    deficitPercentage: {
      name: "Deficit percentage",
      formula: function () {
        return (
          "pH range [" +
          settings.severity.mild.range.lower +
          " to " +
          settings.severity.mild.range.upper +
          " = " +
          settings.severity.mild.deficitPercentage +
          "%] or [" +
          settings.severity.moderate.range.lower +
          " to " +
          settings.severity.moderate.range.upper +
          " = " +
          settings.severity.moderate.deficitPercentage +
          "%] or [" +
          settings.severity.severe.range.lower +
          " to " +
          settings.severity.severe.range.upper +
          " = " +
          settings.severity.severe.deficitPercentage +
          "%]"
        );
      },
      limit: function () {
      ...
      working: function () {
        if (calcVars.patient.severity.isSevere())
          return (
            "[pH " +
            inputs.pH.toFixed(2) +
            "] is in range [" +
            ...
            settings.severity.severe.range.upper +
            "] ==> " +
            calcVars.deficit.percentage() +
            "%"
          );
        if (calcVars.patient.severity.isModerate())
          return (
            "[pH " +
            inputs.pH.toFixed(2) +
            "] is in range [" +
        ...
        if (calcVars.patient.severity.isMild())
          return (
            "[pH " +
            inputs.pH.toFixed(2) +
            "] is in range [" +
        ...
        throw new Error(
          "Unable to show working. inputs.pH check: [" +
            inputs.pH +
            "] isSevere check: [" +
            calcVars.patient.severity.isSevere() +
            "]. isModerate check: [" +
            calcVars.patient.severity.isModerate() +
            "]. isMild check: [" +
            calcVars.patient.severity.isMild() +
            "]. settings.severity check: [" +
            JSON.stringify(settings.severity) +
            "]"
        );
      },
      output: function () {
    ...
    deficitVolume: {
      ...
      limit: function () {
        if (calcVars.deficit.percentage() == 5)
          return settings.caps.deficit5 + "mL (for 5% deficit)";
        if (calcVars.deficit.percentage() == 10)
          return settings.caps.deficit10 + "mL (for 10% deficit)";
      ...
      working: function () {
        var working =
          ...
          inputs.weight +
          "kg] x 10 = " +
          calcVars.deficit.volumeUncapped().toFixed() +
          "mL";
    ...
    bolusVolume: {
      ...
      working: function () {
        var working =
          ...
          inputs.weight +
          "kg] = " +
          calcVars.bolus.volumeUncapped(10).toFixed() +
          "mL";
    ...
    deficitVolumeLessBolus: {
      ...
      working: function () {
        return (
          "[" +
          calcVars.deficit.volume().toFixed() +
          "mL] - [" +
          calcVars.deficit.bolusToSubtract().toFixed() +
          "mL] = " +
          calcVars.deficit.volumeLessBolus().toFixed() +
          "mL"
        );
      },
      output: function () {
    ...
    deficitReplacementRate: {
      ...
      working: function () {
        return (
          ...
          calcVars.deficit.volumeLessBolus().toFixed() +
          "mL] &#247; [48 hours] = " +
          calcVars.deficit.rate().toFixed(1) +
          "mL/hour"
        );
      },
      output: function () {
    ...
    maintenanceVolume: {
      ...
      working: function () {
        ...
        working =
          working +
          " = " +
          calcVars.maintenance.volumeUncapped().toFixed() +
          "mL";
        if (calcVars.maintenance.isCapped())
          working = working + " (exceeds cap)";
        return working;
      },
      output: function () {
    ...
    maintenanceRate: {
      ...
      working: function () {
        return (
          "[" +
          calcVars.maintenance.volume().toFixed() +
          "mL] &#247; [24 hours] = " +
          calcVars.maintenance.rate().toFixed(1) +
          "mL/hour"
        );
      },
      output: function () {
    ...
    startingFluidRate: {
      ...
      working: function () {
        return (
          ...
          calcVars.maintenance.rate().toFixed(1) +
          "mL/hour] = " +
          calcVars.startingFluidRate().toFixed(1) +
          "mL/hour"
        );
      },
      output: function () {
    ...
    insulinRate: {
      ...
      limit: function () {
        if (inputs.insulin == 0.05)
          return (
            settings.caps.insulin005 + " Units/hour (for 0.05 Units/kg/hour)"
          );
        if (inputs.insulin == 0.1)
          return (
            settings.caps.insulin01 + " Units/hour (for 0.1 Units/kg/hour)"
          );
        throw new Error("Unable to select limit");
      },
      working: function () {
        var working =
          ...
          inputs.weight +
          "kg] = " +
          calcVars.insulin.rateUncapped().toFixed(2) +
          " Units/hour";
...
const generate = {
  click: function () {
    //runs on user clicking generate protocol
    if (this.internetExplorer.detect()) {
      //cannot use web worker method if IE
  ...
  showLoading: function () {
    //shows a loading modal until the PDF generation is finished
    var dialog = bootbox.dialog({
      message:
        '<div class="panel panel-primary" style="text-align:center;"><div class="panel-heading">Please wait while the protocol is being generated...</div><div class="panel-body"><div class="row"><div class="col-sm-12"><i class="fa fa-spin fa-cog"></i><img src="/images/pageLoader.gif" alt="Loading..." style="display:block; margin-left:auto; margin-right:auto;" class="img-responsive"></div></div></div><div class="panel-footer">This can take up to a minute.</div></div>',
      closeButton: false,
    });
    return dialog;
  },
  ...
  success: function () {
    ...
    bootbox.dialog({
      message:
        '<div class="panel panel-success" style="text-align:center;"><div class="panel-heading">Protocol generated</div><div class="panel-body">The DKA protocol for ' +
        inputs.name +
        " should now be downloading.</div></div>",
      size: "small",
      closeButton: false,
      backdrop: false,
      buttons: {
  ...
  internetExplorer: {
    //functions relating to PDF generation for IE
    detect: function () {
    ...
    showLoading: function () {
      bootbox.confirm({
        title:
          "<span style='font-size: 24px'>This might take a while...</span>",
        message:
          "The process of generating a protocol can take up to about a minute (and sometimes longer on slow computers) when using Internet Explorer. <br><br><b>During this time your browser may appear to be unresponsive</b><br><br>Use a newer browser such as Chrome, Firefox or Edge for a much faster experience.",
        buttons: {
          cancel: {
    ...
    generate: function () {
      ...
      pdfMake
        .createPdf(docDef)
        .download("DKA Protocol for " + inputs.name + ".pdf"); //generate from the created docdefinition and trigger download
